...
function Rent_cars() {
  ...
  useEffect(() => {
    ...
    const fetchCarDetails = async () => {
      try {
        ...
        const response = await fetch(`https://carrent-backend-r2g6.onrender.com/car_details/${cars_id}`);
        if (!response.ok) {
  ...
  const handleRent = async (type) => {
    ...
    const statusCheck = await fetch(`https://carrent-backend-r2g6.onrender.com/car_details/${cars_id}`);
    if (!statusCheck.ok) {
      Swal.fire({
        icon: "error",
        title: "Error",
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          const response = await fetch("https://carrent-backend-r2g6.onrender.com/user_rent_cars", {
            method: "POST",
